...
3. Ask that verifier to find omitted required topics, unsupported factual claims, unmapped fixed constraints, and broken reverse traceability. Fix confirmed issues or record unresolved gaps in `repodocs/audit/drift-report.md`, then rerun the blind check. Stopping rule: each rerun uses a fresh verifier; stop when a pass returns no blocking or important issues, or when two consecutive passes return only minor ones - fresh attention finds a long tail forever, so "rerun until silent" has no terminating condition. Hard bound: eight passes. If the stopping rule is still unmet after eight passes, stop anyway, record the unmet rule and every unresolved issue in the drift report, and set `verification.blind` to `failed` - capped depth is recorded honestly, never rounded up to passed. Record the number of passes and per-pass issue counts in the drift report so verification depth stays visible. Record `verification.blind: "passed"` with zero unresolved blocking issues only after such a pass; a failed blind check makes the run failed.

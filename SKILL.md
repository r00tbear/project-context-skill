...
6. Review preflight `scope_review` with the user before any auditor is dispatched: an exclusion that contains tracked files needs explicit confirmation that it is deliberate (the answer may legitimately be "exclude it anyway"); a tracked path that is also git-ignored is an inconsistency in its own right - ignoring a tracked path is a no-op that usually means a forgotten `git rm --cached` - record it as a finding; agent instruction files inside an excluded path are routed to the security auditor regardless of exclusions. Never turn this review into an automatic scope change or a name-based skip list.

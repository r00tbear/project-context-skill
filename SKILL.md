...
3. Ask that verifier to find omitted required topics, unsupported factual claims, unmapped fixed constraints, and broken reverse traceability. Classify every issue on two axes: severity (blocking/important/minor) and kind - a **contradiction** (the surface asserts something the repository contradicts, or acting on it breaks a contract) versus an **omission** (something true and useful is absent). Fix confirmed issues or record unresolved gaps in `repodocs/audit/drift-report.md`, then rerun the blind check. Stopping rule: each rerun uses a fresh verifier; stop when a pass reports zero blocking issues and zero important contradictions, or when two consecutive passes return only minor issues. Important omissions from the final pass do not restart the loop: record them in the drift report and disposition them (backlog or debt) like findings - contradictions converge because the text is finite, but completeness against "everything a maintainer might need" does not, so fresh attention finds a new true omission forever. Hard bound: eight passes. If the stopping rule is still unmet after eight passes, stop anyway, record the unmet rule and every unresolved issue in the drift report, and set `verification.blind` to `failed` - capped depth is recorded honestly, never rounded up to passed. Record the number of passes and per-pass issue counts (split contradiction/omission) in the drift report so verification depth stays visible. Record `verification.blind: "passed"` with zero unresolved blocking issues only after such a pass; a failed blind check makes the run failed.
